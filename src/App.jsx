      ...
      {Array.from({ length: 50 }, (_, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            width: sz, height: sz * 1.6, background: color,
...
export default function DominoScore() {
  ...
  const deleteRound = (idx) => {
    ...
    setHistory(recalced);
    const finalTotals = recalced.length > 0
      ? recalced[recalced.length - 1].totals
      ...
      <div style={{ width: "100%", maxWidth: 480, padding: "0 16px", zIndex: 1, position: "relative" }}>
        ...
        {screen === "setup" && (
          <div className="screen-enter" style={{ display: "flex", flexDirection: "column", gap: 14, paddingTop: 20 }}>
            ...
            <div style={{ background: theme.card, borderRadius: 20, padding: 20, border: `1px solid ${theme.cardBorder}`, backdropFilter: "blur(10px)" }}>
              ...
              <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
                {[2, 3, 4].map(n => (
                  <button key={n} className="btn" onClick={() => setNumPlayers(n)} style={{
                    ...
                    background: numPlayers === n ? `linear-gradient(135deg, ${theme.accent}, ${theme.accent2})` : "rgba(255,255,255,0.05)",
              ...
              {Array.from({ length: numPlayers }).map((_, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
                  <div style={{ width: 10, height: 32, borderRadius: 4, background: PLAYER_COLORS[i], boxShadow: `0 0 10px ${PLAYER_COLORS[i]}66`, flexShrink: 0 }} />
                  <input
                    value={playerNames[i]}
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {WINNING_SCORES.map(s => (
                  <button key={s} className="btn" onClick={() => setWinScore(s)} style={{
                    ...
                    background: winScore === s ? `linear-gradient(135deg, ${theme.accent2}, ${theme.accent})` : "rgba(255,255,255,0.05)",
            ...
            <button className="btn" onClick={startGame} style={{
              ...
              boxShadow: `0 8px 32px ${theme.glow}55`, border: "none",
            }}>{tx.startGame} →</button>
          </div>
        )}
          ...
          <div className="screen-enter" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                {Array.from({ length: numPlayers }).map((_, i) => {
                  ...
                  return (
                    <div key={i} style={{
                      ...
                      <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1.5, color: PLAYER_COLORS[i], textTransform: "uppercase", fontFamily: "'DM Sans', sans-serif" }}>
                        {playerNames[i]}
                      </div>
                      <div style={{
                        fontFamily: "'Bebas Neue', serif",
                        fontSize: numPlayers === 2 ? 64 : 48,
                        lineHeight: 1, color: PLAYER_COLORS[i],
                ...
                {Array.from({ length: numPlayers }).map((_, playerIdx) => (
                  <div key={playerIdx} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    ...
                    <input
                      type="number"
                      inputMode="numeric"
                      value={inputs[playerIdx]}
                      onChange={e => setInputs(inp => {
                        const n = [...inp];
                        n[playerIdx] = e.target.value;
                        return n;
                      })}
                      placeholder="0"
                      min="0"
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 4 }}>
                      {[5, 10, 15, 20, 25, 30].map(v => (
                        <button
                          key={v}
                          className="quick-btn"
                          onClick={() => handleQuickAdd(playerIdx, v)}
                          style={{
                            fontSize: 13,
                            padding: "8px 0",
                            border: `1px solid ${PLAYER_COLORS[playerIdx]}22`,
                          }}
                        >
                          {v}
                        </button>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button className="btn" onClick={addScores} style={{
                ...
                color: "#000",
                fontFamily: "'Bebas Neue', serif", fontSize: 20, letterSpacing: 2,
                boxShadow: `0 6px 24px ${theme.glow}55`, border: "none",
              }}>{tx.addScores}</button>
            </div>
        ...
        {screen === "history" && (
          <div className="screen-enter">
            <div style={{ background: theme.card, borderRadius: 20, padding: 16, border: `1px solid ${theme.cardBorder}`, backdropFilter: "blur(10px)" }}>
              <div style={{ fontSize: 10, color: theme.muted, marginBottom: 14, letterSpacing: 2, textTransform: "uppercase", fontFamily: "'DM Sans', sans-serif" }}>
                {tx.scoreHistory}
              </div>
              ...
              {history.length === 0 ? (
                ...
                <>
                  <div style={{ display: "grid", gridTemplateColumns: `24px repeat(${numPlayers}, 1fr) 60px`, gap: 6, marginBottom: 8, padding: "0 4px" }}>
                    <div style={{ fontSize: 10, color: theme.muted }}>#</div>
                    {Array.from({ length: numPlayers }).map((_, i) => (
                      <div key={i} style={{ fontSize: 10, fontWeight: 700, color: PLAYER_COLORS[i], textAlign: "center", letterSpacing: 1, fontFamily: "'DM Sans', sans-serif" }}>
                        {playerNames[i]}
                      </div>
                    ))}
                    <div />
                  ...
                  <div style={{ maxHeight: 450, overflowY: "auto", display: "flex", flexDirection: "column", gap: 4 }}>
                    {history.map((h, idx) => (
                      <div key={idx}>
                        <div style={{
                          display: "grid", gridTemplateColumns: `24px repeat(${numPlayers}, 1fr) 60px`,
                          ...
                          {Array.from({ length: numPlayers }).map((_, i) => (
                            <div key={i} style={{ textAlign: "center" }}>
                              <div style={{ fontFamily: "'Bebas Neue', serif", fontSize: 18, color: PLAYER_COLORS[i], letterSpacing: 1 }}>{h.totals[i]}</div>
                              {h.added[i] > 0 && (
                                <div style={{ fontSize: 9, color: PLAYER_COLORS[i], opacity: 0.55, fontFamily: "'DM Sans', sans-serif" }}>+{h.added[i]}</div>
                              )}
                            </div>
                          ))}
                          <div style={{ display: "flex", gap: 4, justifyContent: "center" }}>
                            <button className="btn" onClick={() => editingRound === idx ? setEditingRound(null) : openEdit(idx)} style={{
                              ...
                              color: editingRound === idx ? "#000" : theme.muted, border: "none",
                            }}>✏️</button>
                            <button className="btn" onClick={() => deleteRound(idx)} style={{
                              padding: "5px 7px", borderRadius: 8, fontSize: 12,
                              background: "rgba(255,60,60,0.12)", color: "#ff6b6b", border: "none",
                            }}>🗑</button>
                          </div>
                        </div>
                          ...
                          <div className="edit-panel" style={{
                            ...
                            <div style={{ display: "flex", gap: 8 }}>
                              ...
                              <button className="btn" onClick={saveEdit} style={{
                                ...
                                background: `linear-gradient(135deg, ${theme.accent}, ${theme.accent2})`,
                                color: "#000",
                                fontFamily: "'Bebas Neue', serif", fontSize: 17, letterSpacing: 2, border: "none",
                              }}>{tx.save}</button>
                            </div>
                          </div>
                        )}
        ...
        {screen === "winner" && winner !== null && (
          <div className="screen-enter" style={{ textAlign: "center", paddingTop: 16 }}>
            ...
            <div style={{ fontFamily: "'Bebas Neue', serif", fontSize: 48, letterSpacing: 4, color: PLAYER_COLORS[winner], textShadow: `0 0 40px ${PLAYER_COLORS[winner]}99`, marginBottom: 4 }}>
              {playerNames[winner]}
            </div>
            ...
            <div style={{ fontFamily: "'Bebas Neue', serif", fontSize: 96, lineHeight: 1, color: theme.accent, textShadow: `0 0 60px ${theme.glow}88`, marginBottom: 24 }}>
              {scores[winner]}
            </div>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button className="btn" onClick={resetGame} style={{
                flex: 1, padding: "16px", borderRadius: 14,
                background: `linear-gradient(135deg, ${theme.accent}, ${theme.accent2})`,
                color: "#000", fontFamily: "'Bebas Neue', serif", fontSize: 18, letterSpacing: 2,
